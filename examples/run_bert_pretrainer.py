...
def train(args, train_dataset, model, tokenizer, featurizer, config):
	...
	train_sampler = RandomSampler(train_dataset, replacement=True, num_samples=len(train_dataset)) if args.local_rank == -1 else DistributedSampler(train_dataset)
	...
	for _ in train_iterator:
		...
		for step, file_data in enumerate(file_iterator):
			...
			example_sampler = RandomSampler(file_data, replacement=True, num_samples=len(file_data)) if args.local_rank == -1 else DistributedSampler(file_data)
